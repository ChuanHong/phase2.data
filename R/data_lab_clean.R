data_lab_clean=function(dat.x.raw, code.dict, nm.patient_num, nm.days_since_admission, nm.value, day){
  dat.sub=dat.x.raw[dat.x.raw[,nm.days_since_admission]==day & dat.x.raw[,"concept_type"]%in%"LAB-LOINC",]
  ...
  dat.sub.wide=dat.sub.wide[,setdiff(colnames(dat.sub.wide), c("LAB-LOINC:48065-7", "LAB-LOINC:48066-5"))]
  ...
  dat.sub.wide[,which(is.na(colnames(dat.sub.wide))!=1)]
}